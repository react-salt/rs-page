import React, { Component } from 'react';

const datasets = [{
  sync_status: 0,
  item_id: '5024278',
  title: '商品名称是什么，一样正常显示，最多显示两行，多余部分省略号表示后面还有东西，不要乱动我的商品，快速导入宝贝信息',
  alias: '2fqanb96l48ch',
  info: '599.00',
  created_time: '2016-04-25 19:01:17',
  bro_uvpv: '0/0',
  num: '0',
  image_url: 'http://dn-kdt-img-test.qbox.me/upload_files/2015/07/01/FvjXIrzNwsxviCcTDh7QDRqzfV2_.jpg?imageView2/2/w/100/h/100/q/75/format/webp',
  buy_url: '',
  stock_num: '60',
  sold_num: 0,
  buy_way: '1',
  tags: [],
  is_display: [
    'koudaitong'
  ],
  is_display_num: 1,
  goods_type: 0,
  is_lock: 0,
  is_used: 0,
  freezing_endtime: '0000-00-00 00:00:00',
  is_edit_lock: 0,
  fx_auth: 1,
  url: 'http://wap.koudaitong.com/v2/goods/2fqanb96l48ch',
  changed_info: [],
  changed_description: '',
  urls: {
    koudaitong: 'http://wap.koudaitong.com/v2/goods/2fqanb96l48ch'
  }
}, {
  sync_status: 0,
  item_id: '5024277',
  title: '商品名称是什么，一样正常显示，最多显示两行，多余部分省略号表示后面还有东西，不要乱动我的商品，快速导入宝贝信息',
  alias: '3f1i9ej23ake9',
  info: '599.00',
  created_time: '2016-04-25 19:01:16',
  bro_uvpv: '0/0',
  num: '0',
  image_url: 'http://dn-kdt-img-test.qbox.me/upload_files/2015/07/01/FvjXIrzNwsxviCcTDh7QDRqzfV2_.jpg?imageView2/2/w/100/h/100/q/75/format/webp',
  buy_url: '',
  stock_num: 59,
  sold_num: 0,
  buy_way: '1',
  tags: [],
  is_display: ['koudaitong'],
  is_display_num: 1,
  goods_type: 0,
  is_lock: 0,
  is_used: 0,
  freezing_endtime: '0000-00-00 00:00:00',
  is_edit_lock: 0,
  fx_auth: 1,
  changed_info: [],
  changed_description: '',
  urls: {
    koudaitong: 'http://wap.koudaitong.com/v2/goods/3f1i9ej23ake9'
  }
}];

export default class WrapperTable extends Component {

  state = {
    limit: 10,
    current: 0,
    total: 101
  }

  onChange(conf) {
    console.log(Object.assign(this.state, conf))
    this.setState(conf)
  }

  render() {
    return (
      <ul>
        {datasets.map(item => <li key={item.item_id}>{item.toString()}</li>)}
      </ul>
    )
  }
}
